...
export async function loadSlashCommands() {
    ...
    for (const scFile of slashCommandFiles) {
        ...
        try {
            ...
            if (!config) {
                Logger.error(
                    `Missing config in slash command "${fileBasename}"`,
                );
                continue;
            }
            ...
            Logger.error(
                `Error loading slash command "${fileBasename}": \n\t${err}`,
            );
        }
    }
        ...
        switch (option.type) {
            case "STRING": {
                commandBuilder.addStringOption((optionBuilder) => {
                    const stringOption =
                        option as SlashCommandStringOptionConfig;
            ...
            case "INTEGER": {
                commandBuilder.addIntegerOption((optionBuilder) => {
                    const integerOption =
                        option as SlashCommandNumberOptionConfig;
